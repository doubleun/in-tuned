...
>(({ className, children, ...props }, ref) => (
  <NavigationMenuPrimitive.Root
    ref={ref}
    className={cn(
      'relative z-10 flex flex-1 items-center justify-center w-full',
      className
    )}
    {...props}
  >
...
const navigationMenuTriggerStyle = cva(
  'inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus:outline-none focus:bg-slate-100 disabled:opacity-50 dark:focus:bg-black/30 disabled:pointer-events-none bg-transparent hover:bg-slate-100 dark:hover:bg-black/20 dark:text-slate-100 dark:hover:text-slate-100 data-[state=open]:bg-slate-50 dark:data-[state=open]:bg-black/30 data-[active]:bg-slate-50 dark:data-[active]:bg-black/30 h-10 py-2 px-4 group w-max'
)
